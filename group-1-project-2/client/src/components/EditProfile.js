...
export default function EditProfile() {                                      //this exports the Edit profile function
  const [redirect, setRedirect] = useState("");                              // this sets the current stae to redirect

  if (!localStorage.getItem("uid")) {                                        //if the user id is not equal to the local storage it redirects the user to the login page
  ...
  //part 2
  useEffect(() => {
    Axios.post("http://localhost:4000/profile", {                          // this posts the user id from the local storage to the url localhost:4000/profile page in the user id field
      uid: localStorage.getItem("uid"),
    }).then((response) => {
      if (typeof response.data.error !== "undefined") {                   // if the user is undefined it returns the error message and redirects the user to the home page
        // redirect to home
  ...
  const onFormSubmit = (e) => {                                          // function that executes on submission of the form
    e.preventDefault();
    ...
    Axios.post("http://localhost:4000/updateuser", formDataObj).then(    // posts the data on the page with url localhost:4000/updateuser page
      (response) => {
        if (typeof response.data.error !== "undefined") {                // if the user is undefined it gives the error message and redirects the user to the home page.
          //redirect to home
          setMesssage(response.data.error);
        } else {
  ...
  return (                                                             //this function returns the form with the Fields Username to be neterd by the user and Email address field for email id to be entered by the user and a submit button to submit the data entered in these feilds by the user.
